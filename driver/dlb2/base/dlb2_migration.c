...
#define DUMMY_CQ_HIST_LIST_BASE		DLB2_MAX_NUM_HIST_LIST_ENTRIES
#define DUMMY_CQ_HIST_LIST_LIMIT	(DUMMY_CQ_HIST_LIST_BASE  \
...
uint32_t read_hist_list_pop_ptr (struct dlb2_hw *hw, uint32_t cq)
{
	uint32_t pop_ptr;
...
uint32_t read_hist_list_push_ptr (struct dlb2_hw *hw, uint8_t cq)
{
	uint32_t push_ptr;
...
int set_hl_pop_push_ptr(struct dlb2_hw *hw, uint8_t cq, uint16_t idx)
{
...
uint16_t num_ldb_inflights (struct dlb2_hw *hw, uint8_t cq)
{
	uint32_t infl_cnt;
				     ...
				     struct dlb2_ldb_queue *queue)
{
	...
	uint32_t num_sn = 64; /* change this based on sn_mode used */
	uint32_t slot_shift, j, k, val;
	uint16_t sn_min, sn_idx;

	src_state = hw->mig_state.src_vm_state;
...
bool dlb2_return_token(struct dlb2_hw *hw, uint8_t cq, bool is_ldb)
{
	...
	pp_addr = os_map_producer_port(hw, cq, is_ldb);
	os_enqueue_four_hcws(hw, hcw, pp_addr);

	if (is_ldb)
        	tkn_cnt = DLB2_CSR_RD(hw, LSP_CQ_LDB_TKN_CNT(hw->ver, cq));
	else
        	tkn_cnt = DLB2_CSR_RD(hw, LSP_CQ_DIR_TKN_CNT(hw->ver, cq));
				  ...
				  struct dlb2_migration_state *src_state)
{
	...
	for (i = 0; i < entries; i++) {
		...
		if (src_state->hlist_state[hl_idx].qtype == ORDERED) {
			...
			pp_addr = os_map_producer_port(hw, src_cq, true);
			     ...
			     struct dlb2_ldb_port *buddy_port)
{
	...
	if (buddy_port) {
		...
		DLB2_CSR_WR(hw, CHP_HIST_LIST_LIM(hw->ver, cq), DUMMY_CQ_HIST_LIST_LIMIT - 1);
        	DLB2_CSR_WR(hw, CHP_HIST_LIST_BASE(hw->ver, cq), DUMMY_CQ_HIST_LIST_BASE);
		DLB2_CSR_RD(hw, CHP_HIST_LIST_BASE(hw->ver, cq));
	}
...
static int pf_send_COMP_T(struct dlb2_hw *hw, uint8_t cq)
{
	...
	pp_addr = os_map_producer_port(hw, cq, true);
			      ...
			      struct dlb2_migration_state *src_state)
{
	...
	for (i = 0; i < src_state->num_ldb_queues; i++) {
		...
		//queue_depth = dlb2_ldb_queue_depth(hw, src_state->ldb_queue[i]);
	...
	for (i = 0; i < src_state->num_ldb_ports; i++) {
		//dummy_cq = src_state->dummy_ldb_port->id.phys_id;
		...
		set_hl_base_limit(hw, ldb_port, NULL);
		DLB2_CSR_WR(hw, CHP_HIST_LIST_POP_PTR(hw->ver, cq), DUMMY_CQ_HIST_LIST_BASE);
		DLB2_CSR_WR(hw, CHP_HIST_LIST_PUSH_PTR(hw->ver, cq), DUMMY_CQ_HIST_LIST_BASE);
				 ...
				 if (hcw->cq_token == 1) {
					...
					if (cnt % 1000 == 0)
						LM_DEBUG_print("[%d]Reading SRC HCW[%d]: 0x%016llx 0x%016llx"
							" at CQ=%d QID = %d qType = %d udata64: %lld\n",
		...
		set_hl_base_limit(hw, ldb_port, ldb_port);
		ndelay(10000);
	}
		...
		while (loop_cnt < 10000) {
			...
			if (dlb2_dir_cq_token_count(hw, dir_port) > tkn_cnt) {
				...
				vqid = vcq;//hcw->qid;
			        ...
			        if (cnt % 500 == 0)
					LM_DEBUG_print("[%d]Reading SRC HCW[%d]: 0x%016llx 0x%016llx"
						" at CQ=%d VQID = %d qType = %d udata64: %lld\n",
				  ...
				  struct dlb2_migration_state *src_state)
{
	...
	for (i = 0; i < src_state->num_ldb_queues; i++) {
		...
		if (queue->sn_cfg_valid == 1) {
			/* FIXME calculate the renq_qid from actual drain process */
			qid_state->renq_qid = 31;  /* The Atomic/Dir Queue between workers and consumer */
			for (k = 0; k < qid_state->num_sn_in_hl; k++) {
				...
				DLB2_CSR_WR(hw, CHP_HIST_LIST_POP_PTR(hw->ver, cq), src_state->hl_ptr[hl_idx]);
				val = read_hist_list_pop_ptr(hw, cq);
				tkn_cnt = dlb2_ldb_cq_token_count(hw, port);
				inflights = num_ldb_inflights(hw, cq);
				LM_DEBUG_print("MIG: QID [%d(%d)] Sending in a COMP for ORD Entry SN = %d via CQ = %d "
				       "hl_idx = %d pop_ptr = 0x%08x inflights: %d"
				       " tkn_cnt: %d\n", qid, vqid, sn, cq, hl_idx, val, inflights, tkn_cnt);
	...
	/* To get the re-enqueue QID */
	/*
	for (k = 0; k< MAX_QID_EMPTY_CHECK_LOOPS; k++) {
		if ( (num_qid_enq_count(qid, Q_ATM) == 0) && (num_qid_enq_count(qid, Q_NALB) == 0) ) {
			printf ("    DEBUG: LDB QID is empty \n");
			break;
		}
	}
	*/
...
bool pf_sch_dummy_ord_hcw(struct dlb2_hw *hw,
			  struct dlb2_ldb_port *port,
			  struct dlb2_ldb_queue *queue)
{
 ...
 */
bool fill_dest_rob_renq(struct dlb2_hw *hw, uint8_t cq, uint8_t qid, uint16_t idx)
{
	...
	uint32_t tkn_cnt;
        ...
        dst_state = hw->mig_state.dst_vm_state;
	...
	pp_addr = os_map_producer_port_maskable(hw, cq, true);
	os_enqueue_four_hcws(hw, hcw, pp_addr);
	LM_DEBUG_print("PP[%d] Writing RENQ HCW[%d] QID = %d qType = %d 0x%04x\n", cq, idx, qid,
		hcw->sched_type, hcw->lock_id);
	return 0;
}
...
static int dlb2_fill_dest_rob_hl(struct dlb2_hw *hw)
{
	...
	num_sn = 64; /* FIXME for general case */
	...
	for (i = 0; i < dst_state->num_ldb_queues; i++) {
		...
		/* Process only the ORD QIDs */
		if (dst_queue->sn_cfg_valid == 1 && dst_state->ldb_qid_state[dst_vqid].num_sn_in_hl > 0) {
			...
			for (j = 0; j < num_sn; j++) {
				/* FIXME: using the first CQ for now.  Need to get a valid CQ assigned to the QID */
				dst_port = dst_state->ldb_port[0];
				dst_cq = dst_port->id.phys_id;
				dst_vcq = dst_port->id.virt_id;

				LM_DEBUG_print("MIG: ROB Processing: num_sn:%d sn_min = %d, j = %d, SN = %d QID = %d(%d) \n",
				...
				if (dst_state->ldb_qid_state[dst_vqid].sn_state[sn].hl_valid) {
					...
					/* Restore the original HL for the drained CQ */
					set_hl_base_limit(hw, dst_port, dst_port);
					set_hl_pop_push_ptr(hw, dst_cq, dst_port->hist_list_entry_base +
							    (hl_idx - dst_state->ldb_cq_state[dst_vcq].hist_list_entry_base));

					LM_DEBUG_print("MIG: ROB FILL: Writing Dummy ORD HCW to CQ=%d hl_idx = %d"
						" (0x%04x) SN = %d QID = %d \n", dst_cq, hl_idx, hl_idx, sn, dst_qid);
					...
					if (pf_sch_dummy_ord_hcw(hw, dst_port, dst_queue))
					  ...
					  if (num_drained < dst_state->ldb_qid_state[qid2].num_drain_rob_hcws) {
					    ...
					    set_hl_base_limit(hw, dst_port, NULL);
					    set_hl_pop_push_ptr(hw, dst_cq, DUMMY_CQ_HIST_LIST_BASE);

					    if (pf_sch_dummy_ord_hcw(hw, dst_port, dst_queue))
						    LM_DEBUG_print("ERR: sch_dummy_ord_hcw Failed \n");
					    /* second, insert a RENQ to complete the first pass */
					    if (fill_dest_rob_renq(hw, dst_cq, qid2, num_drained))
...
bool pf_sch_dummy_hcw(struct dlb2_hw *hw, uint8_t cq, uint16_t hl_idx, uint16_t tkn_cnt)
{
	struct dlb2_hcw hcw_mem[8], *hcw;
        void __iomem *pp_addr;
	uint8_t qid;
	 ...
	 */
        LM_DEBUG_print("tkn_cnt: %d, dst_state->ldb_cq_state[cq].tkn_cnt: %d\n",
			tkn_cnt, dst_state->ldb_cq_state[cq].tkn_cnt);
	//if (tkn_cnt >= dst_state->ldb_cq_state[cq].tkn_cnt)
	//	hcw->cq_token = 1;
	...
	pp_addr = os_map_producer_port(hw, cq, true);
	LM_DEBUG_print("Writing SRC HCW at HL[%d] HCW: 0x%016llx 0x%016llx to PP =%d "
		"PP addr:%p QID = %d qType = %d, udata64: %lld\n",
		hl_idx, *(uint64_t *)hcw, *((uint64_t *)hcw + 1), cq, pp_addr,
...
bool fill_dest_hl(struct dlb2_hw *hw,
		  struct dlb2_ldb_port *dst_port)
{
	...
	/* Need to start writing the destination CQ with the current GEN bit from the SRC */
	DLB2_CSR_WR(hw, CHP_LDB_CQ_WPTR(hw->ver, dst_cq), (dst_state->ldb_cq_wptr[dst_vcq] & 0x800));
	...
	LM_DEBUG_print("dst_port->hist_list_entry_limit : 0x%08x , dst_state->ldb_cq_state[dst_vcq].hist_list_entry_base : 0x%08x\n",
		dst_port->hist_list_entry_limit - 1, dst_state->ldb_cq_state[dst_vcq].hist_list_entry_base);
	...
	for (i = 0; i < inflights; i++) {
		...
		if (dst_state->hlist_state[hl_idx].qtype != ORDERED) {
			...
			if (pf_sch_dummy_hcw(hw, dst_cq, hl_idx, tkn_cnt))
	...
	//DLB2_CSR_WR(hw, LSP_CQ_LDB_TKN_CNT(hw->ver, dst_cq), dst_state->ldb_cq_state[dst_vcq].tkn_cnt);
	tkn_cnt = dlb2_ldb_cq_token_count(hw, dst_port);
...
bool pf_sch_dummy_dir_hcw(struct dlb2_hw *hw,
			  struct dlb2_dir_pq_pair *port)
{
	struct dlb2_hcw hcw_mem[8], *hcw;
	void __iomem *pp_addr;
...
bool restore_dest_tokens(struct dlb2_hw *hw,
 ...
 */
bool dlb2_fill_dest_vas_hl(struct dlb2_hw *hw)
{
	struct dlb2_migration_state *dst_state;
	struct dlb2_dir_pq_pair *dir_port;
	struct dlb2_ldb_port *port;
	...
	for(i = 0; i < dst_state->num_ldb_ports; i++) {
		if (fill_dest_hl(hw, dst_state->ldb_port[i]))
	...
	for (i = 0; i < dst_state->num_dir_ports; i++) {
		...
		LM_DEBUG_print("MIG: Disabling DST CQ: %2d \n", cq);
		dlb2_dir_port_cq_disable(hw, dir_port);
		restore_dest_tokens(hw, dir_port);
	}
	LM_DEBUG_print("\n");
...
bool fill_dest_qes_dir(struct dlb2_hw *hw,
		       uint8_t pp,
	...
	for(i = 0; i < dst_state->dir_qid_state[dst_vqid].num_drain_hcws; i++) {
		...
		if (i % 500 == 0 || i == dst_state->dir_qid_state[dst_vqid].num_drain_hcws - 1) {
			...
			LM_DEBUG_print("[%d]After writing SRC HCW: 0x%016llx 0x%016llx using PP: %d with QID = %d"
				" qType = %d udata64: %lld :: na_eq: %d, VAS DIR Credits: %d \n", i,
...
bool fill_dest_qes_ldb(struct dlb2_hw *hw,
		       uint8_t pp,
		       struct dlb2_ldb_queue *dst_queue)
{
	struct dlb2_migration_state *dst_state;
	...
	for(i = 0; i < dst_state->ldb_qid_state[dst_vqid].num_drain_hcws; i++) {
		...
		if (i % 500 == 0 || i == dst_state->ldb_qid_state[dst_vqid].num_drain_hcws - 1) {
			enq = DLB2_CSR_RD(hw, LSP_QID_LDB_ENQUEUE_CNT(hw->ver, dst_qid));
			val = DLB2_CSR_RD(hw, CHP_CFG_LDB_VAS_CRD(domain_id));
			LM_DEBUG_print("[%d]After writing SRC HCW: 0x%016llx 0x%016llx using PP: %d with QID = %d"
				" qType = %d udata64: %lld :: na_eq: %d, VAS LDB Credits: %d \n", i,
				*(uint64_t *)hcw_out, *((uint64_t *)hcw_out + 1), pp, hcw_out->qid,
				hcw_out->sched_type, hcw_out->data, enq, val);
		}
	}
...
bool dlb2_fill_dest_vas_qes(struct dlb2_hw *hw)
{
	...
	for (i = 0; i < dst_state->num_ldb_queues; i++) {
		...
		if (fill_dest_qes_ldb(hw, pp, dst_state->ldb_queue[i]))
			LM_DEBUG_print("FILL QE FAIL \n");
	}
	for (i = 0; i < dst_state->num_dir_ports; i++) {
		pp = dst_state->dir_port[0]->id.phys_id; //Using first port to enqueue QEs
		if (fill_dest_qes_dir(hw, pp, dst_state->dir_port[i]))
			LM_DEBUG_print("FILL QE FAIL \n");
	}
	return 0;
}
...
bool dlb2_restore_state_vas(struct dlb2_hw *hw)
{
	...
	for (i = 0; i < dst_state->num_ldb_ports; i++) {
		...
		set_hl_base_limit(hw, dst_ldb_port, dst_ldb_port);
...
bool dlb2_resume_vas(struct dlb2_hw *hw,
		     bool vdev_req,
...
{
	struct dlb2_migration_state *src_state, *dst_state;
	uint32_t i;
	...
	LM_DEBUG_print("MIG: List of SRC LDB CQs: (Phy ID, Virt ID) \n");
	for(i = 0; i < src_state->num_ldb_ports; i++)
		LM_DEBUG_print("%2d, %2d\n", src_state->ldb_port[i]->id.phys_id,
			src_state->ldb_port[i]->id.virt_id);
	LM_DEBUG_print("\n");
	LM_DEBUG_print("MIG: List of DEST LDB CQs: (Phy ID, Virt ID) \n");
	for(i = 0; i < dst_state->num_ldb_ports; i++)
		LM_DEBUG_print("%2d, %2d\n", dst_state->ldb_port[i]->id.phys_id,
			dst_state->ldb_port[i]->id.virt_id);
	LM_DEBUG_print("\n");


	/* DIR Port */
	LM_DEBUG_print("MIG: Number of DIR CQs to migrate = %d \n", src_state->num_dir_ports);
	LM_DEBUG_print("MIG: List of SRC DIR CQs: (Phy ID, Virt ID) \n");
	for(i = 0; i < src_state->num_dir_ports; i++)
		LM_DEBUG_print("%2d, %2d\n", src_state->dir_port[i]->id.phys_id,
			src_state->dir_port[i]->id.virt_id);
	LM_DEBUG_print("\n");
	LM_DEBUG_print("MIG: List of DEST DIR CQs: (Phy ID, Virt ID) \n");
	for(i = 0; i < dst_state->num_dir_ports; i++)
		LM_DEBUG_print("%2d, %2d\n", dst_state->dir_port[i]->id.phys_id,
			dst_state->dir_port[i]->id.virt_id);
	LM_DEBUG_print("\n");


	/* LDB Queue */
	LM_DEBUG_print("MIG: Number of LDB Queues to migrate = %d \n", src_state->num_ldb_queues);
	LM_DEBUG_print("MIG: List of SRC LDB Queues: (Phy ID, Virt ID) \n");
	for(i = 0; i < src_state->num_ldb_queues; i++)
		LM_DEBUG_print("%2d, %2d\n", src_state->ldb_queue[i]->id.phys_id,
			src_state->ldb_queue[i]->id.virt_id);
	LM_DEBUG_print("\n");
	LM_DEBUG_print("MIG: List of DEST LDB Queues: (Phy ID, Virt ID) \n");
	for(i = 0; i < dst_state->num_ldb_queues; i++)
		LM_DEBUG_print("%2d, %2d\n", dst_state->ldb_queue[i]->id.phys_id,
			dst_state->ldb_queue[i]->id.virt_id);
	LM_DEBUG_print("\n");
	LM_DEBUG_print("====================================================\n");
}
                                  ...
                                  unsigned int vdev_id)
{
	...
	for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
		...
		src_state->ldb_port[vcq] = ldb_port;
		...
		LM_DEBUG_print("MIG: CQ_ADDR for SRC_CQ[%d] upper=0x%x, lower= 0x%x\n",
                        ldb_port->id.phys_id, src_state->ldb_cq_addr[vcq].up,
	...
	src_state->num_ldb_queues = 0;
		...
		dlb2_read_src_queue_state(hw, ldb_queue);

	}

	/* Print the migration state saved */
	dlb2_print_mig_state(hw);
                                  ...
                                  unsigned int vdev_id)
{
	...
	for (i = 0; i < DLB2_NUM_COS_DOMAINS; i++) {
		...
		dst_state->ldb_port[vcq] = ldb_port;
			  	...
			  	unsigned int vdev_id)
{
	...
	LM_DEBUG_print("--------------------------------------------------------------------\n");
	ret = dlb2_prepare_migration(hw, vdev_req, vdev_id);
	if (ret) {
	...
	LM_DEBUG_print("-----------------------------------------------------------------------\n");
	ret = dlb2_rerun_pending_src_comps(hw);
	if (ret) {
			   ...
			   unsigned int vdev_id)
{
	...
	LM_DEBUG_print("-----------------------------------------------------------------------\n");
	ret = dlb2_prepare_resumption(hw, vdev_req, vdev_id);
	if (ret) {
		LM_DEBUG_print("Prepare Failed \n");
		return ret;
	}
	...
	LM_DEBUG_print("-----------------------------------------------------------------------\n");
	/* Resume DST VAS */
	if (dlb2_resume_vas(hw, vdev_req, vdev_id)) {
