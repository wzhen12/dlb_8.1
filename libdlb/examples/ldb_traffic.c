...
static void *rx_traffic(void *__args)
{
    thread_args_t *args = (thread_args_t *) __args;
    dlb_event_t events[NUM_EVENTS_PER_LOOP];
    int ret, nfds = 0, epoll_fd, num_rx = 0;
    struct epoll_event epoll_events;
    int epoll_retry = EPOLL_RETRY;
    int64_t num_loops, i, num_mismatch = 0;
	...
	/* Receive the events */
	    ...
	    else if (events[j].recv.udata64 != num_rx + j ||
    ...
    printf("[%s()] Received %ld events, num_mismatch: %d\n",
...
static void *worker_fn(void *__args)
{
	...
	if (total % 100000 == 0)
		printf("[%s] Received and forwarded events : %d\n", __func__, total);
    }
...
int parse_args(int argc, char **argv)
{
    ...
    for (;;) {
        c = getopt_long(argc, argv, "n:d:w:f:hc:l:e:p:m:s:", long_options, &option_index);
        if (c == -1)
            break;
